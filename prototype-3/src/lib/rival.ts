...
function outfield(placements: string[], removed: string[] = []): { id: string; position: Position }[] {
  const roster = ROSTER.filter(p => !isGK(p.position) && !removed.includes(p.id)).map(p => ({ id: p.id, position: p.position }))
...
export function myTeamAxes(activeAxes: string[], placements: string[], removed: string[] = []): number[] {
  const ids = outfield(placements, removed)
...
export function myUnits(placements: string[], removed: string[] = []): { DEF: number; MID: number; ATT: number } {
  const ids = outfield(placements, removed)
...
export function mostExposed(rival: RivalTeam, placements: string[], removed: string[] = []): Exposure | null {
  ...
  ROSTER.filter(p => positions.includes(p.position) && !removed.includes(p.id)).forEach(p => consider(p.id, p.name, p.position, p.graduating))
