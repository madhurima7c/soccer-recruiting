...
// ═════════════════════════ PITCHSIDE additions ═════════════════════════════
// Standalone field platform: my starting XI on the 3D field, the coach score
// (1–5, shown as the stroke around every player node), and full rival XIs.

// My 2026 starting XI — one roster id per PITCH_SPOTS index (1-4-3-3)
export const MY_XI: string[] = [
  'r1',  // GK  Tess Okafor
  'r3',  // CB  Marisol Vega (graduating)
  'r4',  // CB  Anneke Visser
  'r6',  // LB  Skye Tanaka (graduating)
  'r7',  // RB  Carmen Ibarra
  'r9',  // DM  Noa Lindqvist (graduating)
  'r11', // CM  Georgia Whitfield (graduating)
  'r12', // CM  Sana Yoshida
  'r14', // LW  Zora Bektaš (graduating)
  'r15', // RW  Callie Runningwater
  'r17', // CF  Bridget Halloran (graduating)
]

// Coach score — the staff's own 1–5 scale (spring review for roster players,
// latest viewing rating for recruits). This is the number the stroke encodes.
export const COACH_SCORE: Record<string, number> = {
  r1: 4, r2: 3, r3: 5, r4: 4, r5: 3, r6: 4, r7: 4, r8: 3, r9: 4,
  r10: 3, r11: 5, r12: 4, r13: 3, r14: 4, r15: 4, r16: 3, r17: 5, r18: 3,
}
export function coachScoreOf(id: string): number | undefined {
  const rec = RECRUITS.find(r => r.id === id)
  if (rec) return rec.ratings.length ? rec.ratings[rec.ratings.length - 1] : undefined
  return COACH_SCORE[id]
}

// Rival starting XIs — same PITCH_SPOTS order. Scout grade is OUR staff's
// 1–5 read of that opponent player from film (not their coach's number).
export interface RivalPlayer { name: string; grade: number; note?: string }
export const RIVAL_XI: Record<string, RivalPlayer[]> = {
  stanton: [
    { name: 'A. Whitcombe', grade: 4 },
    { name: 'P. Okonkwo', grade: 4 },
    { name: 'S. Marchetti', grade: 3 },
    { name: 'D. Liang', grade: 3 },
    { name: 'R. Castellanos', grade: 4 },
    { name: 'I. Novak', grade: 5, note: 'The 6 — everything routes through her. Deny the entry pass and they stall.' },
    { name: 'M. Delacroix', grade: 5, note: 'Left 8. Breaks lines on the half-turn; presses poorly the other way.' },
    { name: 'T. Braithwaite', grade: 3 },
    { name: 'K. Asante', grade: 4 },
    { name: 'J. Lindgren', grade: 3 },
    { name: 'E. Vasquez', grade: 4, note: 'False 9 movement — drops in, CBs must not follow.' },
  ],
  redwood: [
    { name: 'B. Kowalski', grade: 3 },
    { name: 'N. Achebe', grade: 4 },
    { name: 'H. Sørensen', grade: 4, note: 'Steps out aggressively — space in behind her every time.' },
    { name: 'C. Munro', grade: 3 },
    { name: 'F. Reyes', grade: 3 },
    { name: 'L. Van Bergen', grade: 4 },
    { name: 'G. Tanaka', grade: 4, note: 'Sets the press traps. Their PPDA runs through her triggers.' },
    { name: 'W. McBride', grade: 3 },
    { name: 'O. Diallo', grade: 5, note: 'Transition winger — 40 yards in two touches when they win it.' },
    { name: 'S. Hargrove', grade: 3 },
    { name: 'Y. Petrova', grade: 4 },
  ],
  mesaverde: [
    { name: 'D. Hollis', grade: 3 },
    { name: 'M. Iversen', grade: 4, note: 'Attacks the GK on every restart. Physical mismatch hunter.' },
    { name: 'A. Gutierrez', grade: 3 },
    { name: 'T. Okafor-Bell', grade: 3 },
    { name: 'J. Wren', grade: 3 },
    { name: 'P. Salido', grade: 3 },
    { name: 'K. Bjornstad', grade: 4 },
    { name: 'R. Amaechi', grade: 3 },
    { name: 'C. Fontaine', grade: 5, note: 'Left winger — serves early, back post, every single time. Force her inside.' },
    { name: 'V. Stancu', grade: 4, note: 'Right side. Same early-service pattern; weaker on her left.' },
    { name: 'L. Beaumont', grade: 4, note: 'Target 9 — wins the first ball; the 10 feeds on knockdowns.' },
  ],
}

// ═══════════════════ PITCHSIDE v2 additions ═══════════════════════════════
// Two more rivals (5 total), season-level team stats for the plain-English
// comparison rows, and simpler explanations for every term on screen.

RIVALS.push(
  {
    id: 'olympia', name: 'Olympia Pacific', short: 'OLY', record: '10-7-2 (’25)', formation: '4-4-2',
    color: '#2563EB',
    styleTags: ['sit deep', 'fast breaks', 'patient without the ball'],
    axes: [72, 40, 45, 70, 55, 40],
    extra: { 'Press resistance': 42, 'Work rate': 70 },
    stats: { possession: 41, ppda: 13.5, crossesP90: 14, setPiecePct: 20 },
    units: { DEF: 70, MID: 52, ATT: 66 },
    threatAxis: 'Defending',
    threatWhy: 'They give you the ball, wait for a mistake, and break at full speed. What decides it: how fast you recover when you lose it high.',
    scout: { by: 'luke', on: 'Jul 3', note: 'Two flat banks, zero risk, then four players sprint the moment they win it. Every goal I watched came inside 12 seconds of a turnover. If our fullbacks both push up, we lose this on the counter.', tags: ['rest defense', 'no cheap giveaways', 'beware minute 60+'] },
  },
  {
    id: 'bellfield', name: 'Bellfield State', short: 'BFS', record: '9-6-4 (’25)', formation: '3-5-2',
    color: '#DB2777',
    styleTags: ['physical', 'set-piece heavy', 'long throws'],
    axes: [58, 42, 48, 74, 80, 50],
    extra: { 'Press resistance': 60, 'Work rate': 78 },
    stats: { possession: 47, ppda: 9.8, crossesP90: 18, setPiecePct: 34 },
    units: { DEF: 74, MID: 58, ATT: 60 },
    threatAxis: 'Aerial',
    threatWhy: 'A third of their goals come from corners, free kicks, and long throws. Every dead ball near your box is a real chance for them.',
    scout: { by: 'priya', on: 'Jun 22', note: 'Biggest team in the conference and they know it. Three rehearsed corner routines, a long-throw specialist, and two 6-foot forwards. Open play is ordinary — if we defend restarts well, we win.', tags: ['defend restarts', 'no fouls near the box', 'win first contact'] },
  },
)

RIVAL_XI['olympia'] = [
  { name: 'S. Marsh', grade: 3 },
  { name: 'E. Odum', grade: 4 },
  { name: 'K. Fitzgerald', grade: 3 },
  { name: 'L. Barrios', grade: 3 },
  { name: 'T. Nakamura', grade: 3 },
  { name: 'A. Percy', grade: 3 },
  { name: 'D. Kessler', grade: 4, note: 'Launches every break — first pass always forward.' },
  { name: 'M. Oyelaran', grade: 3 },
  { name: 'R. Vintner', grade: 5, note: 'Fastest player in the conference. Do not let her run at the back line.' },
  { name: 'C. Delgadillo', grade: 3 },
  { name: 'H. Strand', grade: 4, note: 'Clinical on 2-v-2 breaks. Finishes early, near post.' },
]
RIVAL_XI['bellfield'] = [
  { name: 'P. Woodruff', grade: 3 },
  { name: 'G. Matlock', grade: 4, note: 'Long-throw specialist — reaches the six-yard box flat.' },
  { name: 'N. Eze', grade: 4 },
  { name: 'B. Halvorsen', grade: 3 },
  { name: 'J. Cotter', grade: 3 },
  { name: 'F. Adjei', grade: 3 },
  { name: 'V. Romanova', grade: 3 },
  { name: 'A. Whitlock', grade: 3 },
  { name: 'O. Bankole', grade: 4 },
  { name: 'D. Krieger', grade: 5, note: '6-foot forward, wins everything in the air. Their corner target.' },
  { name: 'S. Todorova', grade: 4 },
]

// Season-level rows for the plain-English comparison (per game, last season)
export interface TeamSeason { gf: number; ga: number; aerialPct: number; returning: number }
export const RIVAL_SEASON: Record<string, TeamSeason> = {
  stanton:   { gf: 2.1, ga: 0.9, aerialPct: 48, returning: 9 },
  redwood:   { gf: 1.9, ga: 1.2, aerialPct: 55, returning: 7 },
  mesaverde: { gf: 1.7, ga: 1.3, aerialPct: 62, returning: 8 },
  olympia:   { gf: 1.5, ga: 1.0, aerialPct: 52, returning: 10 },
  bellfield: { gf: 1.4, ga: 1.1, aerialPct: 64, returning: 6 },
}
export const MY_SEASON: TeamSeason & { possession: number; ppda: number; setPiecePct: number } = {
  gf: 1.6, ga: 1.1, aerialPct: 46, returning: 5,
  possession: 54, ppda: 9.0, setPiecePct: 18,
}

// Plainer explanations — assume the reader has never seen the term before.
Object.assign(EXPLAIN, {
  gf: 'Goals scored per game last season. Above 2.0 is a strong attack; below 1.3 struggles to score.',
  ga: 'Goals allowed per game last season. Under 1.0 is a strong defense; above 1.5 leaks.',
  winback: 'How many passes a team lets the opponent make before challenging for the ball. A SMALL number means they hunt the ball back immediately; a big number means they retreat and wait.',
  air: 'Share of headers and 50/50 aerial contests won. Matters most when the other team crosses a lot or is dangerous on corners.',
  experience: 'How many of last season\'s starters come back next fall. More returners = chemistry already built; fewer = a team still figuring itself out.',
  coachscore: 'Your staff\'s own 1–5 grade for this player, set at the spring review (roster) or the latest viewing (recruits). It\'s drawn as the ring around every player on the field.',
  percentile: 'Where this player ranks against collegiate recruits at the same position. 75 means better than 75 out of 100 of them. 50 is dead average — the dashed line.',
  positionavg: 'The dashed shape = the average recruit at her position. Anywhere the solid shape sticks out past the dashed one, she\'s above average.',
  firsttouch: 'How cleanly she controls a ball played to her, especially under pressure. Bad first touch = turnovers in tight spaces.',
  passingT: 'Short and medium passing accuracy and weight — can teammates play off her passes without breaking stride?',
  decision: 'Does she choose the right option — pass, carry, shoot, clear — at game speed? Improves with coaching; slower to fix than technique.',
  spatial: 'Does she see space and danger before the ball arrives? Scanning, positioning between the lines, knowing what\'s behind her.',
  pacekmh: 'Top sprint speed. 32+ km/h is fast for the college game.',
  accel: 'Time to cover the first 10 and 30 meters from standing. The 10m split matters more than top speed for defenders and forwards alike.',
  strengthP: 'Holds her ground in shoulder-to-shoulder duels and shields the ball. Hard to teach; bodies mature at different ages.',
  agilityP: 'Change of direction at speed. Wingers and fullbacks live on this.',
  distance: 'Total ground covered per match. 10km+ is a high-motor player.',
  hir: 'Meters covered at high intensity (sprinting/near-sprint) per match — the work-rate number that separates joggers from pressers.',
  ncaa: 'NCAA academic eligibility status. "Cleared" means she can play Division 1 right now; anything else has paperwork between her and the field.',
})
