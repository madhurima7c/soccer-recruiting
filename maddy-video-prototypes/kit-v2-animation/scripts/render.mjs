...
const outputFile = path.join(outDir, "kit-v2-1080p.mp4");
const port = 3010;
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__kitV2Export?.duration);
    await page.evaluate(async () => {
      await document.fonts.ready;
    });

    const { duration } = await page.evaluate(() => window.__kitV2Export);
    console.log(`Capturing ${duration} frames at 1920×1080…`);

    for (let frame = 0; frame < duration; frame += 1) {
      await page.evaluate((f) => window.__kitV2Export.setFrame(f), frame);
