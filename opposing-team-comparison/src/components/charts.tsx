...
export interface RadarSeries { label: string; values: number[]; color: string; dash?: boolean; noFill?: boolean }
export function RadarChart({ axes, series, size = 220, showMedian = true, axisValues, valueColor }: {
  axes: string[]; series: RadarSeries[]; size?: number; showMedian?: boolean
  axisValues?: number[]                       // shown under each axis label
  valueColor?: (v: number) => string          // percentile-band color for values
}) {
  ...
  return (
    <svg width={size} height={size} role="img"
      ...
      {series.map((s, si) => (
        <g key={si}>
          <polygon points={poly(s.values)} fill={s.noFill || s.dash ? 'none' : s.color} fillOpacity={0.16}
            stroke={s.color} strokeWidth={s.dash ? 1.5 : 2} strokeLinejoin="round"
            strokeDasharray={s.dash ? '4 4' : undefined} />
          {!s.dash && s.values.map((v, i) => {
            const [x, y] = pt(i, v)
      ...
      {axes.map((a, i) => {
        const [x, y] = pt(i, axisValues ? 124 : 118)
        return (
          <g key={a}>
            <text x={x} y={axisValues ? y - 5 : y} textAnchor="middle" dominantBaseline="middle"
            ...
            {axisValues && (
              <text x={x} y={y + 7} textAnchor="middle" dominantBaseline="middle"
                ...
                fill={valueColor ? valueColor(axisValues[i]) : 'currentColor'}>
                {axisValues[i]}
              </text>
            )}
          </g>
